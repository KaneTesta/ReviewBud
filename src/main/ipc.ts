...
import { fetchFileSource, fetchPullRequest, fetchRecentPullRequests, fetchRecentRepositories, fetchSymbolContext, replyToPullRequestDiscussion, searchRepositories, setPullRequestFileViewed, submitPullRequestReview } from "./github.js";
import { ReviewStorage } from "./storage.js";
import { explainSnippetWithCodex } from "./codex.js";
...
import type { DraftReviewComment, DraftReviewSubmission, PullRequestDiscussionReplyRequest, PullRequestFileViewedRequest, PullRequestReviewSubmissionRequest, ReviewNote, SnippetExplanationRequest, SymbolContextRequest } from "../shared/types.js";
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle("pr:setFileViewed", async (_event, request: PullRequestFileViewedRequest) =>
    setPullRequestFileViewed(request),
  );

  ipcMain.handle("pr:explainSnippet", async (_event, request: SnippetExplanationRequest) =>
    explainSnippetWithCodex(request, app.getPath("userData")),
  );
